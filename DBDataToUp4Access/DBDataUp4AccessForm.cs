...
using DBDataToUp4Access.Properties;
...
{
    public partial class DBDataUp4AccessForm : Form
    {
        ...
        private string CURR_DIR = "";//本地地址
        private string CURR_RM = "";//远程地址
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(res))
                {
                    ...
                    if (rInfo.Data != null)
                    {
                        ...
                        DBToolsAccess.DBLink = CURR_DBConf;
                        bool bok = checkConfDB();
                        if (bok)
                        {
                            DBToolsAccess.insertOrUpDate(configM.Sid, configM.Bgtime);
                        }
                        checkLogDB();
        ...
        private void checkLogDB() {
            try
            {
                bool bok = DBToolsAccess.checkLogTable();
            }
            catch (Exception ex) {
        ...
        private bool checkConfDB()
        {
            ...
            try
            {
                bok = DBToolsAccess.checkConfigTable();
            }
            catch (Exception ex)
            {
        ...
        private void TestDBLink(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(DBToolsAccess.DBLink))
                    DBToolsAccess.DBLink = CURR_DBConf;
                string bgtime = DBToolsAccess.GetSearchBgTime(configM.Sid);
        ...
        public bool CheckServer(bool bshow = false) {
            ...
            try
            {
                string res = Tools.HttpPostInfo(CURR_URI + ICL.API_KEY, "type=200&json=" + s1);
        ...
        private void LocalSave(object sender, EventArgs e)
        {
            ...
            iniFile.IniWriteValue(ICL.LOCL_LAB, ICL.LOCL_DIR, local);
            iniFile.IniWriteValue(ICL.LOCL_LAB, ICL.LOCL_RM, rm);
            CURR_DBID = dbid;
            CURR_OPR = opr;
            CURR_SCM = scm;
            CURR_DIR = local;
        ...
        {
            //string rmFile = tb_rm.Text;
            //string locDir = tb_loc.Text;
            //ToolMoveFiles.CopyFile(rmFile, locDir);
            ExcuteTask();
        ...
        private void DBDataUpForm_Load(object sender, EventArgs e)
        {
            ...
            if (iniFile.ExistINIFile()) {
                ...
                string loc = iniFile.IniReadValue(ICL.LOCL_LAB, ICL.LOCL_DIR);
                CURR_DIR = loc;
                tb_loc.Text = loc;
                string rm = iniFile.IniReadValue(ICL.LOCL_LAB, ICL.LOCL_RM);
                CURR_DIR = rm;
                tb_rm.Text = rm;
        ...
        private void makeUIEnable(bool enable) {
            ...
            tb_loc.Enabled = enable;
            tb_rm.Enabled = enable;
        ...
        private void DBDataUp4AccessForm_SizeChanged(object sender, EventArgs e)
        {
            //判断是否选择的是最小化按钮
            if (WindowState == FormWindowState.Minimized) {
                //隐藏任务栏区图标
        ...
        private void DBDataUp4AccessForm_FormClosing(object sender, FormClosingEventArgs e)
        {
