...
{
    public partial class PMDDataFixForm : Form
    {
        ...
        private void PMDDataFixForm_Load(object sender, EventArgs e)
        {
            DateTime maxDate = DateTime.Now.AddDays(-1);
            //bg_time.MaxDate = maxDate;
            //ed_time.MaxDate = DateTime.MaxValue;
        ...
        private void ExcuteFixed(object sender, EventArgs e)
        {
            if (configM == null) {
                MessageBox.Show("请重新获取配置信息！");
                return;
            }
            if (string.IsNullOrEmpty(configM.Sid))
            {
                MessageBox.Show("请重新获取配置信息！");
                return;
            }
            log_box.Clear();
            string bgdate = bg_time.Text;
            string eddate = ed_time.Text;
            DateTime bgd = DateTime.Parse(bgdate);
            DateTime edd = DateTime.Parse(eddate);
            int k = DateTime.Compare(bgd, edd);
            if ( k<=0) {

                edd = edd.AddDays(1);
                eddate = edd.ToString("yyyy-MM-dd");
                bgdate += " 00:00:00";
                eddate += " 00:00:00";
                if (job == null)
                {
                    job = new ProdJob(CURR_URI, configM);
                }
                else {
                    job.url = CURR_URI;
                    job.setConfigM(configM);
                }
                job.cURR_SCM = CURR_SCM;
                job.cURR_OPR = CURR_OPR;
                job.cURR_DBID = CURR_DBID;
                job.bgtime = bgdate;
                job.edtime = eddate;

                //sql = string.Format(sql, bgdate, eddate);
                UseWaitCursor = true;
                BeginInvoke(
                    new Action(() => {
                        //QueryData(sql);
